...
In addition several non-class functions and symbols can also be imported, they
are grouped into tags with lower-case names. You should only include the ones
you need. In this case C<run> is included automatically as it is in the
...
C<label> is a pointer to a character string to label the widget with or
C<undef>. If not given it defaults to C<undef>. The string is C<not> copied,
FLTK assummes it is static. For example, this:
...
overloading to make short names for get/set methods. The underlying libraries
use mutators so I brought them along for the ride.

Almost all of these set/get pairs are very fast and short inline functions and
...
to the parent of itself, in this case it is set to null because the window has
no parent.  You may also find it useful to call
